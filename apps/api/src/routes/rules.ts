...
rules.get("/", async (c) => c.json(await listRules(c.env.DB, effectiveOwner(c).owner)));
...
rules.post("/", async (c) => {
  ...
  const row = await createRule(c.env.DB, writeOwner(c), {
    field: b.field,
    op: b.op,
    value: b.value.trim(),
    action: b.action,
...
rules.post("/update", async (c) => {
  ...
  await updateRule(c.env.DB, effectiveOwner(c).owner, b.id, b.enabled ? 1 : 0);
...
rules.post("/delete", async (c) => {
  ...
  await deleteRule(c.env.DB, effectiveOwner(c).owner, b.id);
  return c.json({ ok: true });
});

// Re-run rules over existing inbound mail.
rules.post("/run", async (c) => {
  const touched = await applyRulesToExisting(c.env.DB, writeOwner(c));
